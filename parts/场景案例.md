...
3. higress+nacos配置问题导致mcp client大量ping请求透传到业务系统
4. hikari切换druid后启动报错：Property 'sqlSessionFactory' or 'sqlSessionTemplate' are required
   - 不是普通 Spring Boot 数据源，而是 ShardingSphere 数据源
   - 之前使用 Hikari 时能成功，原因通常是：ShardingSphere + Hikari 是官方主推组合，自动适配最完整
   - druid-spring-boot-starter 会尝试参与 Spring Boot 的 DataSource 自动装配
   - spring.shardingsphere.datasource也会参与 Spring Boot 的 DataSource 自动装配
   - 两套数据源体系发生竞争，最终导致 ShardingSphere 没创建出有效 DataSource，MyBatis 拿不到 SqlSessionFactory
   - 解决方案：只引入 druid 本体，不使用 druid-spring-boot-starter
