...
2. Nacos 是如何既支持 CP 架构也支持 AP 架构的
   - Config 模块 —— CP 模型：配置中心必须保证强一致，使用 Raft 协议
   - Naming 模块 —— 默认 AP：服务注册与发现的场景服务实例上下线频繁，偶尔不一致问题不大，更强调可用性
     - 在服务注册时可以指定ephemeral: false改成 CP 模式
3. Spring Cloud Gateway 和 Zuul 2.x 的差别不大,也是通过过滤器来处理请求。  
   - 更推荐使用 Spring Cloud Gateway 而非 Zuul,Spring Cloud 生态对其支持更加友好。
4. 常用的网关有 Spring Cloud Gateway、Kong、APISIX
...
7. Spring Cloud Gateway如何基于ip或者userId进行定向路由
   - 实现自定义KeyResolver Bean，在route中指定它
8. UUID 的优缺点:存储消耗空间大/无序(非自增)会严重影响数据库性能
9. 分布式id生成方式有哪些?
    - **Snowflake雪花算法**
      - 不依赖外部数据库，完全在内存中生成。
...
10. 基于数据库的号段模式时，如何防止并发请求获取到同一批号段
...
13. Redisson提供了一个专门用来监控和续期锁的Watch Dog(看门狗),默认情况下,每过10秒,看门狗就会执行续期操作,将锁的超时时间设置为30 秒
...
16. Seata的AT模式两阶段提交和传统的两阶段提交有什么区别?
...
17. Seata AT 模式下，默认的隔离级别是“读未提交（Read Uncommitted），怎么防止脏读和脏写
...
24. BASE 是 Basically Available（基本可用）、Soft-state（软状态） 和 Eventually Consistent（最终一致性） 三个短语的缩写。
    - BASE 理论本质上是对 CAP 的延伸和补充，更具体地说，是对 CAP 中 AP 方案的一个补充。AP 方案只是在系统发生分区的时候放弃一致性，而不是永远放弃一致性。在分区故障恢复后，系统应该达到最终一致性。
25. Zookeeper选举leader过程
...
28. 常见的网关Spring Cloud Gateway和Zuul
...
32. RPC //todo
33. SPI（Service Provider Interface） 的具体原理是这样的：我们将接口的实现类放在配置文件中，我们在程序运行过程中读取配置文件，通过反射加载实现类。这样，我们可以在运行的时候，动态替换接口的实现类。和 IoC 的解耦思想是类似的。
34. Dubbo 提供的负载均衡策略有哪些？
...
51. Azure AD 和 Service Principal 的区别
...
57. kubectl和helm的区别
    - kubectl 是 Kubernetes 的官方命令行客户端，直接和 Kubernetes API Server 通信，操作单个资源
    - Helm 是 Kubernetes 的 包管理器，管理一整套 Kubernetes 资源的安装、升级、回滚（最终也是调用 Kubernetes API）
    - Helm 并不是替代 kubectl，而是构建在 Kubernetes API 之上的高级封装
    - Helm 的包叫做 Chart，本质是带变量的 Kubernetes YAML 模板集合
58. 怎么使用 Nginx Ingress 进行灰度发布
...
59. 怎么使用istio进行蓝绿部署
    - **概念**: 维护两个独立的生产环境（Blue-当前版，Green-新版）。通过修改 Istio 路由规则，将实时流量从 Blue瞬间切换到 Green，实现零停机发布和即时回滚。
    - **核心步骤与配置**:
      1. **定义两个 Deployment**: 为 `blue` 和 `green` 版本创建独立的 Deployment。它们必须共享一个 `app` 标签，但使用不同的 `version` 标签。
         ```yaml
         ...
         spec:
           template:
             ...
             spec:
               ...
               - image: my-app:v1.0
         ---
         # deployment-green.yaml (类似, 但 image: my-app:v2.0 且 version: green)
         ```
      2. **创建统一的 Service**: 定义一个 Service，通过 `app` 标签同时选中 `blue` 和 `green` 的 Pods，为路由提供一个稳定的入口。
         ```yaml
         apiVersion: v1
         kind: Service
         metadata:
         ...
         spec:
           selector:
             app: my-app # 选中所有版本的 Pods
         ```
      3. **定义路由规则 (DestinationRule & VirtualService)**: `DestinationRule` 用来定义 `blue` 和 `green` 两个子集。`VirtualService` 则控制流量，初始状态下将 100% 流量指向 `blue`。
         ```yaml
         ...
         spec:
           hosts: ["*"]
           gateways: [my-gateway] # 你的 Istio Gateway
           http:
           - route:
             - destination:
                 host: my-app-service
                 subset: blue # 100% 流量到 blue
               weight: 100
         ```
      4. **执行切换**: 部署 `green` 版本后，只需修改 `VirtualService` 中的 `subset` 字段并重新应用，即可将流量瞬间切换到新版本。
         ```yaml
         # 只需修改 VirtualService 的 http.route.destination.subset
         ...
           http:
           - route:
             - destination:
                 host: my-app-service
                 subset: green # <-- 唯一需要修改的地方
               weight: 100
         ...
         ```
60. 蓝绿部署、灰度发布、金丝雀发布的区别
    - 蓝绿部署
      - 关键：全量切换、零停机
      - 发布逻辑：
        - 新代码在 Green 跑顺了
        - 网关（Istio）直接把 100% 的流量从 Blue 切换到 Green
        - 如果 Green 出问题，瞬间切回 Blue
    - 金丝雀发布
      - 关键：渐进式、风险控制、样本观察
      - 发布逻辑：
        - 先切 5% 的流量给新版（金丝雀）
        - 观察这 5% 的用户是否报错、延迟是否增加
        - 如果没有问题，逐步增加比例：10%... 50%... 100%
    - 灰度发布
      - 关键：人群画像、特定条件、业务驱动
      - 发布逻辑：
        - 不是随机抽 10% 流量，而是精准筛选： 
        - 只有 北京地区 的用户能看到新版
        - 只有 VIP 等级 > 5 的用户能看到新版
        - 只有 Android 用户 能看到新版
61. Kustomize是什么
    - Kustomize 是一个开源的配置管理工具，可以在不修改原始 YAML 的情况下，针对不同环境（开发、测试、生产）定制不同的配置
    - kustomize 通过 overlay 叠加的方式，制定不同 patch，在部署时选择使用
    - 可以随时 fork 出一个新版本，也可以创建新的 overlay 将应用推向新的环境
62. 旧版DestinationRule & VirtualService的单service方案 vs Gateway API 的双service 实现蓝绿部署/金丝雀发布
    - Istio 社区大力推行 Gateway API，核心目的就是为了消除 DestinationRule 这种繁琐的中间层。通过直接在 HTTPRoute 里指定不同的 backendRefs（即不同的 Service）实现最纯粹、最透明的流量管理
    - 旧版方案流量分配规则散落在 VirtualService 和 DestinationRule 两个地方，排查链路长。新版HTTPRoute 直接指向后端名，一眼就能看清流量去哪了
    - 在旧版方案中 K8s 原生监控（如 Prometheus/Grafana）很难区分同一 Service 下不同 Subset 的流量指标。新版本方案每个 Service 有独立的 ClusterIP，所有监控看板都能自动按 Service 名分类
63. git pull和git fetch有什么区别
    - git fetch 从远程仓库下载最新提交记录，但不会自动合并到当前分支
    - git pull 会下载远程更新，并立即合并到当前分支
    - git pull = git fetch + git merge
64. git分支策略
65. 怎么使用multi-stage build（多阶段构建）减少镜像体积
    - 只把“运行时必需的产物”复制到最终镜像，而把“构建时需要的工具链和中间文件”全部丢弃。
    - Stage 1：构建阶段(有完整的构建环境，会下载依赖，会编译代码，会生成 target/*.jar，它只是一个“中间容器”)
      ```
      FROM maven:3.8 AS build
      ```
    - Stage 2：运行阶段(全新的最终镜像，只有 JRE，只拷贝一个最终产物)
      ```
      FROM eclipse-temurin:17-jre-alpine
      COPY --from=build /app/target/*.jar app.jar
      ```
66. CSI Driver是如何工作的
    - 创建 Service Principal，授权访问 Key Vault
    - 在 Kubernetes 中创建 Secret
      ```
      kubectl create secret generic kvcreds-xxx \
      --from-literal=clientid=<appId> \
      --from-literal=clientsecret=<password>
      ```
    - Pod 通过 nodePublishSecretRef 引用它
      ```
      nodePublishSecretRef:
        name: "kvcreds-hk02-u-intl-rbac04"
      ```
    - CSI Driver会在运行时读取这个 K8s Secret，Azure Provider 用 SP 换 Token，访问AKV，拿到 SecretProviderClass 中声明的 objects
      - 在 Node 上创建一个 tmpfs 内存目录 
      - 把两个 secret 写成文件 
      - 挂载进容器
        ```
        /app/secrets/
           ...
           └── u-app-rbac-gn-pw01
        ```
67. ENTRYPOINT 和 CMD 的区别
    - ENTRYPOINT = 容器的“固定主命令”
    - CMD = 传给主命令的“默认参数”
    - Docker 启动容器时，本质执行 <ENTRYPOINT> <CMD>
    - 一般组合使用：
      ```
