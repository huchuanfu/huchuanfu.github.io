...
8. 基于数据库的号段模式时，如何防止并发请求获取到同一批号段
...
11. Redisson提供了一个专门用来监控和续期锁的Watch Dog(看门狗),默认情况下,每过10秒,看门狗就会执行续期操作,将锁的超时时间设置为30 秒
...
14. Seata的AT模式两阶段提交和传统的两阶段提交有什么区别?
...
15. Seata AT 模式下，默认的隔离级别是“读未提交（Read Uncommitted），怎么防止脏读和脏写
...
22. BASE 是 Basically Available（基本可用）、Soft-state（软状态） 和 Eventually Consistent（最终一致性） 三个短语的缩写。
    - BASE 理论本质上是对 CAP 的延伸和补充，更具体地说，是对 CAP 中 AP 方案的一个补充。AP 方案只是在系统发生分区的时候放弃一致性，而不是永远放弃一致性。在分区故障恢复后，系统应该达到最终一致性。
23. Zookeeper选举leader过程
...
26. 常见的网关Spring Cloud Gateway和Zuul
...
30. RPC //todo
31. SPI（Service Provider Interface） 的具体原理是这样的：我们将接口的实现类放在配置文件中，我们在程序运行过程中读取配置文件，通过反射加载实现类。这样，我们可以在运行的时候，动态替换接口的实现类。和 IoC 的解耦思想是类似的。
32. Dubbo 提供的负载均衡策略有哪些？
...
49. Azure AD 和 Service Principal 的区别
