...
9. Bean初始化方法执行顺序
   - @PostConstruct -> afterPropertiesSet() -> initMethod, @PreDestroy -> DisposableBean.destroy() -> destroyMethod
...
10. BeanFactoryPostProcessor和BeanPostProcessor的区别 
    - BeanFactoryPostProcessor用于Bean 定义加载之后,Bean 实例化之前进行修改。
...
17. 为什么 Spring 默认不回滚 Checked Exception
    - Spring 事务设计的哲学是：Checked Exception 被认为是“可预期的业务异常”，RuntimeException 被认为是“不可恢复的系统异常”
    - Checked Exception → 业务层已经预期并处理 → 默认不回滚
    - RuntimeException → 系统错误 → 默认回滚
18. application context refresh执行成功就视为liveness,application and command-line runners执行成功才视为readiness
19. EnvironmentPreparedEvent -> ContextPreparedEvent -> StartedEvent -> Runner -> ReadyEvent
20. PropertySourceLoader策略模式实现YamlPropertySourceLoader和PropertiesPropertySourceLoader
21. Spring中的设计模式:
    - Spring
...
29. @Autowired 和 @Resource 的区别是什么
...
34. SpringBoot 是如何实现自动装配的
    ...
    - SpringBoot 2.7版本之后变为 META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports 文件
    - 指定装配顺序：@AutoConfigureBefore/@AutoConfigureAfter/@AutoConfigureOrder
35. Spring Security OAuth2
    - OAuth2 是一个授权协议，允许第三方应用获得有限的访问权限。
    - 四个角色：
      - Resource Owner (资源拥有者/用户)
      - Resource Server (资源服务器，托管受保护资源)
      - Client (客户端，请求资源的第三方应用)
      - Authorization Server (认证服务器，发放令牌)
    - 常用授权模式 (Grant Types)：
      - Authorization Code (授权码模式，最安全，最常用)
      - Client Credentials (客户端模式，用于服务间通信)
      - Refresh Token (刷新令牌)
      - (Password 和 Implicit 模式已不再推荐使用)
    - Spring 生态现状：
      - 旧的 Spring Security OAuth2 项目已弃用。
      - 认证服务器推荐使用 Spring Authorization Server。
      - 资源服务器和客户端功能已集成到 Spring Security 5+ 中。
    - JWT (JSON Web Token) 常用于 OAuth2 中作为令牌格式，包含用户信息和签名，实现无状态认证。
36. SSO 场景中为什么弃用在 URL 中显式传递 token（如 access_token）
    - Token 暴露面过大，前端 JS 可直接读取，任何中间节点都可能拿到它
    - 会将高价值凭证暴露在浏览器、日志和中间节点等不可控环境中，存在严重的泄露和重放风险
    - 如果 access_token 都敢放在 URL 里，refresh_token 只能不发（导致频繁登录）or也放 URL（安全灾难）
37. Server-Side LB和Client-Side LB有什么区别
    ```
    Traditional Server-Side LB (Nginx, F5):
    Client → [LB] → Service Instances
              ↑
    Single point that decides routing
    
    Spring Cloud LoadBalancer (Client-Side):
    Client (with LB library) → Service Instances
      ↑
    Each client makes its own routing decision
    ```
38. Spring Cloud中Trace ID vs Span ID (micrometer+zipkin)
    - Trace ID: One per entire request flow across all services
    - Span ID: Changes for each service or operation
39. Spring Boot 启动过程中的 SpringApplicationRunListeners
    - SpringApplicationRunListeners是监听器集合管理者（多播器Multicaster），Listener 的调度器
    ```
    ...
    ```
40. MyBatis一级缓存和二级缓存
    - 一级缓存：SqlSession，事务级缓存，默认开启
      - 一个@Transactional方法内就是一个SqlSession，属于事务内部优化
      - 本质就是Map<CacheKey, Object>
      - 一级缓存失效条件
        - update/insert/delete会清空，避免脏数据
        - 事务结束，SqlSession关闭，事务结束缓存就消失
        - 不同SqlSession之间缓存不共享
    - 二级缓存：Mapper Namespace，默认关闭
      - 在Mapper xml中使用<cache/>或Mapper接口中使用@CacheNamespace开启缓存
      - 本质只是一个本地 Map<Object, Object> cache
      - 致命问题
        - 缓存粒度粗：update/insert/delete会清空整个namespace缓存，性能很差
        - 分布式问题：多实例场景下，二级缓存存在各自JVM内存中，不会同步，会造成脏数据
      - 一般不用 MyBatis 二级缓存，而是使用Redis
41. Spring Cloud 核心组件
    - 服务发现：Eureka (or Consul/Nacos)
      - @EnableEurekaServer
    ...
    - 网关：Spring Cloud Gateway
      ...
      ```
    - 负载均衡：Spring Cloud LoadBalancer（Ribbon is deprecated）
      - 在RestTemplate/WebClient上使用@LoadBalanced
      ```java
      ...
      ```
    - 断路器：Resilience4j（Hystrix is deprecated）
      ```java
    ...
    - 链路追踪：Micrometer Tracing
      - Micrometer 会生成+在 HTTP Header 中传播 TraceId 和 SpanId
      - Zipkin 负责接收、存储、查询、展示 trace
      ```yml
      management:
        tracing:
          sampling:
      ...
      ```
42. k8s中替代spring cloud组件
    - Eureka ≈ K8s Service Discovery
    - Spring Cloud Gateway ≈ Ingress Controller
    - Config Server ≈ ConfigMaps/Secrets
    - Resilience4j ≈ Service Mesh (Istio) features
    - Spring Cloud LoadBalancer ≈ K8s Service (built-in LB)