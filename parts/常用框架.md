...
35. SSO 场景中为什么弃用在 URL 中显式传递 token（如 access_token）
...
36. Server-Side LB和Client-Side LB有什么区别
    ```
    ...
    ```
37. Spring Cloud中Trace ID vs Span ID (micrometer+zipkin)
    - Trace ID: One per entire request flow across all services
    - Span ID: Changes for each service or operation
38. Spring Boot 启动过程中的 SpringApplicationRunListeners
