...
2. 为什么不能直接以"字符"或"单词"作为模型的输入单元
    - **字符 (Character)**：
        - **语义信息稀疏**：单个字符（如 'a', 'p', 'p', 'l', 'e'）不包含独立的语义，模型需要组合大量字符才能理解一个概念，增加了学习难度。
        - **序列过长**：同样的文本，用字符表示会产生非常长的序列，导致计算量和内存占用急剧增加，尤其是在处理长篇文章时。
    - **单词 (Word)**：
        - **词表爆炸 (Vocabulary Explosion)**：语言中单词数量庞大，且有各种形态变化（如复数、时态），再加上专有名词、拼写错误等，会导致词表规模巨大，难以管理。
        - **未登录词 (Out-of-Vocabulary, OOV)**：模型无法处理词表中不存在的词，如新出现的网络用语、专业术语或罕见词，泛化能力受限。
3. BPE（Byte Pair Encoding）算法解决了什么问题
    - BPE 旨在**平衡词表规模和语义表达能力**，是对“字符”和“单词”两种极端分词方式的折中。
    - **解决 OOV 问题**：它能将任意未登录词拆分为已知的“子词”单元，保证所有文本都能被正确编码，增强了模型的泛化能力。
    - **控制词表大小**：通过迭代合并最高频的字节对，BPE 能够用相对较小的词表（如 32k-128k）覆盖几乎所有文本。常见词本身作为- 个 token，而罕见词则由几个子词 token 构成，实现了高效的编码压缩。
    - **保留语义信息**：相比字符，BPE 生成的子词（如 "encoding" -> "encod", "ing"）往往保留了更多的构词法和语义信息。
4. 自注意力机制（Self-Attention）的核心思想是什么
    - 核心思想是**让模型在处理序列中的每个元素（如单词）时，能够动态地计算序列内所有其他元素对当前元素的重要性**，并据此加权聚合信息，从而更好地理解上下文关系。
    - 它为序列中的每个 token 计算三个向量：**查询 (Query, Q)**、**键 (Key, K)** 和 **值 (Value, V)**。
        - **Q** 代表当前 token，用于发起查询。
        - **K** 代表序列中所有 token，用于被查询，以确定与 Q 的相关性。
        - **V** 代表序列中所有 token 的内容信息。
    - 通过计算 Q 和所有 K 的点积（相似度），得到**注意力分数 (Attention Score)**，这个分数决定了应该在多大程度上关注其他每个 token。
    - 将注意力分数进行 Softmax 归一化后，乘以对应的 V，再求和，就得到了当前 token 融合了全局上下文信息的新表示。
    - 这种机制**忽略了 token 之间的距离**，可以直接捕捉长距离依赖关系，且易于并行计算，是 Transformer 模型成功的关键。
5. Positional Encoding主要解决了什么问题
    - 主要解决了**自注意力机制本身无法感知序列顺序**的问题。
    - Transformer 的核心 Self-Attention 是一种“集合”处理机制，它对输入 token 的位置不敏感。如果没有位置信息，模型会认为 "猫追狗" 和 "狗追猫" 是完全一样的，这显然是错误的。
    - Positional Encoding 的作用就是**向模型输入中注入关于 token 位置的绝对或相对信息**。
    - 它通过一个固定的数学公式（通常是不同频率的 `sin` 和 `cos` 函数）为每个位置生成一个独特的向量，然后将这个位置向量加到对应 token 的词嵌入向量上。
    - 这样，即使两个相同的 token 出现在不同位置，它们最终的输入向量也是不同的，使得模型能够区分词序，理解句子的语法结构。
6. 智能体范式
...
18. 上下文候选构造
    ...
    - 添加对话历史(仅保留最近的 N 条)
    - 添加自定义信息包
19. 如何防止大模型重复输出（endless repetitions）：
    - Temperature
      - 太低（≤0.3）：分布极度尖锐，生成行为 ≈ greedy decoding，一旦某个 token / 片段在当前上下文下成为最优解，它会在后续上下文中持续保持最优 → 无限复读。
      - 太高（≥1.0）：虽然不重复，但语义漂移、语言混杂概率上升
      - 0.6–0.7 是一个“既能打破确定性，又不牺牲连贯性”的折中区间
    - TopP
      - TopP 控制的是模型每一步“还能考虑多少种不同的续写可能性”
      - 当 TopP 过小：候选 token 集合高度固定，很容易反复落入同一语义/句法路径
      - 一般设置TopP ≥ 0.8：持续保留足够多的“逃生分支”，防止模型在某一步被锁死在单一路径
    - TopK=20
      - TopK的实际意义是在 TopP 选出的“概率合理区间”内，再做一次质量裁剪
      - TopK 过小（≤5）：行为接近 greedy，重复概率显著上升
      - TopK 过大（≥100）：引入大量低质量 token，与 Temperature / TopP 叠加后可能损害稳定性
    - MinP=0
      - MinP 的规则：如果某个 token 的概率 < MinP，则该 token 被直接禁止参与采样，避免“强行续写”
      - MinP > 0：当上下文变得困难时，模型可能只有极少数 token 概率高于 MinP，重复风险上升
      - MinP = 0 表示“不启用这条规则”，不禁止任何 token（因为概率不可能 < 0）
    - presence_penalty（0.6–1.2）
      - 只要一个 token 出现过，它在后续步骤的 logit 就会被统一减去一个常数（presence_penalty 的效果：第一次出现：不惩罚，第二次还想用：成本上升，越用越贵）
      - 无限重复的本质是：模型不断选择“已经说过、而且很安全”的 token
      - 当 presence_penalty ≥ 1.5–2.0：高频功能词（的、是、the、and）也被惩罚，模型可能产生不自然同义替换导致语法稳定性下降
