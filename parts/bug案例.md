...
1. SSE流式请求大模型服务，等待1分钟后会报错
   ...
   - 然后怀疑EventSource的超时设置有默认覆盖（因为实际调用是EventSource.connect(okHttpClient)发起的），尝试创建Request的时候通过tag方法设置timeout
   ...
   - 将readTimeout设置为超过1分钟的值，等待1分钟后必定超时（不符合预期），且抛出的异常类型与刚才10s时的不一样（非TimeoutException），而是服务端返回非法数据流之类的
   - 得出初步结论：根因是由于服务提供方导致的，疑似网关之类的默认超时设置，强制中断连接
   - 提报服务提供方检查，果然是因为由于近期切换了集群，而新集群环境的网关默认1分钟超时，改成3分钟后问题得以解决
