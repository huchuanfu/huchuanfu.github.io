...
10. 隐式转换
    - MySQL 在比较不同类型时，会优先将字符串转换为数值，但会带来数据正确性和索引稳定性风险，例如'abc'会被转换成0
    - 字段是 VARCHAR 而查询条件是数字时，MySQL 会将字段转换为数值再比较，这会导致索引失效并可能产生错误匹配
    - 如果一个 SQL 依赖 MySQL 的隐式类型转换才能“正常工作”，那它迟早会在生产环境出问题
    - 开启严格 SQL_MODE 能防止“脏数据写进去”，但不能防止“脏 SQL 被执行”
11. 索引底层数据结构选型
...
17. 正确建立索引的一些建议
    - 被频繁更新的字段应该慎重建立索引
      - 如果一个字段查询频繁，那无论是否更新频繁都应该建立索引，因为查询提升的效率远大于索引维护的成本
    - 限制每张表上的索引数量
    - 尽可能的考虑建立联合索引而不是单列索引
    - 尽可能在区分度大的字段上建立索引
18. MySQL IN和NOT IN会不会走索引？
    - IN 通常会走索引，当元素小于1000以内的时候
    - NOT IN 通常不会走索引，取决于表数量大小，Index is designed for finding matches, not non-matches
    - NOT IN 优化器判断user_id != 1 ≈ 命中绝大多数行，使用索引的“选择性”极低，成本模型更倾向于 type = ALL
    - NOT IN 可以改写成LEFT JOIN with NULL check
    ```sql
    -- Instead of: WHERE user_id NOT IN (1, 5, 10)
    SELECT o.*
    FROM orders o
    LEFT JOIN (SELECT 1 AS id UNION SELECT 5 UNION SELECT 10) excluded
    ON o.user_id = excluded.id
    WHERE excluded.id IS NULL;
    ```
    - 从设计层面考虑，使用“标志位”替代排除逻辑
    - Negative conditions (!=, NOT IN, NOT EXISTS)通常都很难走索引，建议使用正向条件进行索引查询
19. B树和B+树的区别
...
20. MyISAM 引擎和 InnoDB 引擎都是使用 B+Tree,有什么区别
...
21. 在使用XA分布式事务时,InnoDB 存储引擎的事务隔离级别必须设置为 SERIALIZABLE
22. 解决幻读的方法:将事务隔离级别调整为 SERIALIZABLE/在可重复读的事务级别下,给事务操作的这张表添加 Next-key Lock/表锁
23. redo log(重做日志)保证事务的持久性,undo log(回滚日志)来保证事务的原子性。
    MySQL 数据库的数据备份、主备、主主、主从都离不开binlog,需要依靠 binlog 来同步数据,保证数据一致性。
24. MVCC 的实现
    - 创建一个新事务后会创建一个快照(Read View),当用户在这个事务中要读取某个记录行的时候,
...
42. VARCHAR(100)和 VARCHAR(10)的区别是什么
    - 虽说 VARCHAR(100)和 VARCHAR(10)能存储的字符最大长度不同，但二者存储相同的字符串，所占用磁盘的存储空间其实是一样的。不过，VARCHAR(100) 会消耗更多的内存。
43. DATETIME 和 TIMESTAMP 的区别是什么
    - DATETIME 类型没有时区信息，TIMESTAMP 和时区有关。
44. MySQL 基础架构
45. redo log（重做日志）、binlog（归档日志）、undo log（回滚日志）
...
54. WHERE col != NULL 和 col IS NOT NULL是同一个意思吗
    - 在 SQL 中，比较运算的结果有3种：TRUE/FALSE/UNKNOWN
    - col = NULL / col != NULL → UNKNOWN
    - WHERE 条件只会保留TRUE的行，因此结果永远为空，但不会报错
    - WHERE id NOT IN (SELECT id FROM t2)，如果 t2.id 存在任何一条 NULL，整个查询返回空结果
55. Mysql主从复制延迟
    - 主节点写入数据后，从节点一般10-100ms延迟同步
    - 仅同步增量binlog，不是整个文件
    - 可通过SHOW SLAVE STATUS，检查Seconds_Behind_Master: 0 (ideal), <1 (good), >5 (investigate!)
    - Parallel Replication (MySQL 8.0+)：This allows slave to apply multiple transactions in parallel → Faster sync
    - Prometheus + Grafana监测，当延迟异常增大时预警
56. Mysql主从复制方式
    - Asynchronous Replication (Default - Fastest)：Master writes → Returns success immediately → Slave catches up independently，10-100ms
    - Semi-Synchronous Replication (Safer)：Master writes → Waits for AT LEAST ONE slave to confirm → Then returns success，20-200ms
    - Group Replication / MySQL InnoDB Cluster (Strongest)：Master writes → Waits for majority of nodes to agree → Then returns success，50-500ms
57. ShardingSphere中如何解决读从节点请求延迟问题
    - 使用HintManager.setWriteRouteOnly强制路由到master节点去读
    - @Transactional内的读请求会自动路由到master节点
58. Mysql的快照读和当前读
    - 快照读（Snapshot Read）：是为了实现 读写不冲突，让普通的查询不用等修改操作，普通的 SELECT 不加锁，读取的是历史版本（Undo Log），遵循 Read View 的可见性规则
    - 当前读 (Current Read)：是为了实现 写写不冲突（或写写串行化），UPDATE、DELETE、INSERT 以及特殊的 SELECT ... FOR UPDATE 或 SELECT ... LOCK IN SHARE MODE会加锁，且读取的是该记录最新的已提交版本。
59. 在 MySQL RR 级别下，幻读通常发生在**“跨越快照”**的操作中。最经典的例子是：
    - 事务 A：执行 SELECT * WHERE id = 5，结果为空。
    - 事务 B：插入一条 id = 5 的数据并提交。
    - 事务 A：由于快照读，再次 SELECT 依然为空（看似没问题）。
    - 事务 A：执行 UPDATE SET name = 'Gemini' WHERE id = 5。
    - 关键点：UPDATE 是当前读。它会击穿快照，发现 id = 5 居然存在！于是成功更新了这行，并将该行的隐藏字段 DB_TRX_ID 改成了事务 A 的 ID。
    - 事务 A：再次 SELECT * WHERE id = 5。
    - 灵异事件：由于这行数据的事务 ID 现在属于事务 A 自己，根据 MVCC 规则，它变得可见了。原本不存在的记录，现在“幻影”般地出现了。
60. MySQL如何解决幻读？
    - 方案一：使用“当前读”加锁（间隙锁 Gap Lock）
      - 如果你在事务一开始就想封锁某个范围，不让别人插入，你应该使用手动加锁的查询： SELECT * FROM user WHERE id > 10 FOR UPDATE
    - 方案二：升级隔离级别到 Serializable（串行化）
      - 所有的 SELECT 都会被隐式地转换为 SELECT ... LOCK IN SHARE MODE，读写完全串行化
61. 排他锁（X 锁）和共享锁（S 锁）的区别
    - SELECT ... FOR UPDATE，也叫写锁，阻止其他事务的 写 和 加锁读
    - SELECT ... LOCK IN SHARE MODE，也叫读锁，允许其他事务 读，但阻止 写
62. S 锁导致的死锁案例：
    - 事务 A：执行 SELECT ... LOCK IN SHARE MODE (拿到 S 锁)。
    - 事务 B：也执行 SELECT ... LOCK IN SHARE MODE (也拿到 S 锁，因为 S 锁兼容)。
    - 事务 A：尝试 UPDATE 这行。它需要把 S 锁升级为 X 锁，但发现事务 B 占着 S 锁，于是 A 等待 B。
    - 事务 B：也尝试 UPDATE 这行。它也需要升级为 X 锁，但发现事务 A 占着 S 锁，于是 B 等待 A。
    - 结果：死锁（Deadlock）。
      - 如果接下来的操作是 UPDATE：请直接使用 SELECT ... FOR UPDATE。它能一步到位地排他，避免锁升级导致的死锁
      - 在大多数互联网业务（如高并发的 HR 系统、电商）中，我们优先使用乐观锁（UPDATE ... WHERE max_id = old_value），其次是 FOR UPDATE。SHARE MODE 除非在处理复杂的递归查询或外键约束，否则极少使用
63. 如果事务 A 正在写，事务 B 也在写，但它们修改的不是同一行，而是相邻的两行，在什么特殊情况下它们依然会互相阻塞
    - 当 UPDATE 语句不仅仅针对一行确定的“唯一记录”时，锁的范围就会发生**“横向扩张”**
      - 非唯一索引的“等值查询”：WHERE age = 10，除了给 age = 10 的行加锁，InnoDB 还会给 (10 之前的间隙) 和 (10 到 20 之间的间隙) 加锁。
      - 唯一索引的“范围查询”：WHERE id > 5 AND id < 10，InnoDB 会扫描到第一个不满足条件的行（假设是 id=12），并将 (5, 12] 整个区间锁住。
      - 删除（DELETE）**不存在**的记录：WHERE id = 8; (假设 ID 为 8 的行不存在，而数据库中有 5 和 10)，会在 (5, 10) 这个间隙加上间隙锁。（如果 id=8 存在，情况就完全不同，这时执行的是行锁而非间隙锁）
    - 尽量使用主键/唯一索引等值修改：在主键上 UPDATE WHERE id = 10，锁会退化为行锁，不会有间隙锁
64. InnoDB 存储引擎自动检测并处理死锁机制
    - InnoDB 内部维护着一个名为 Wait-for Graph（等待图） 的数据结构
      - 节点：代表当前的事务
      - 边：如果事务 A 正在等待事务 B 释放锁，就画一条从 A 指向 B 的边
      - 检测逻辑：死锁检测器会定时（或在发生锁等待时）扫描这张图。如果图中出现了环路（Cycle），就意味着发生了死锁
    - 一旦检测到环路，InnoDB 不会随便杀掉一个事务，而是会进行“成本核算”，通常会选择回滚事务日志（Undo Log）量最小的那个事务
    - 被牺牲的事务会收到著名的错误代码：1213 - Deadlock found when trying to get lock; try restarting transaction
65. 间隙锁死锁是怎么形成的
    - 间隙锁（Gap Lock）之间是兼容的，但间隙锁与插入意向锁（Insert Intention Lock）是冲突的
66. 数据库主键使用自增会有什么问题
    - 多库多表无法保证唯一性
    - 存在热点写问题：聚簇索引按主键顺序存储，新数据永远插入“索引末尾”，所有写请求集中在最后一个数据页，导致页锁竞争严重