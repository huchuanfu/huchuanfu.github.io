...
16. RabbitMQ 的事务机制局限于生产者和 RabbitMQ 之间，无法覆盖到消费者，也没有 Kafka 和 RocketMQ 中的全局事务支持
17. 在 Kafka 或 RocketMQ 中，使用事务消息模式时，通常是生产者的本地事务与消息发送的原子性，而不是与消费者的本地事务同时绑定。可以使用Kafka+saga模式确保在消费者的本地事务失败时通过补偿机制回滚前面的操作。
18. 延迟消息实现方案
...
19. Kafka 和 RocketMQ 的零拷贝技术 (Zero Copy)
