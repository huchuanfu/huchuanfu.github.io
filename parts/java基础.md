...
40. synchronized的锁消除和锁粗化
    - 两者都发生在JIT编译期间
    - 锁消除（Lock Elimination）：如果 JVM 通过逃逸分析（Escape Analysis）证明某个对象不会被其他线程访问，那么该对象上的锁可以直接去掉。（比如对于局部变量的StringBuffer）
    - 锁粗化（Lock Coarsening）：如果 JVM 发现频繁对同一对象反复加锁/解锁，它会把多个小锁合并成一个大锁。（比如for循环里对同一个对象synchronized）
41. JVM常用参数调优
    - 显式指定堆内存–Xms和-Xmx
    - 显式新生代内存 -Xmn
...
42. Metaspace的NonClass和Class分别指什么
...
50. Class.forName 和 .loadClass 方法的区别
...
53. 并发标记要解决什么问题？并发标记带来了什么问题？如何解决并发扫描时对象消失问题？
...
54. G1 的压缩为什么会停顿？ G1 在清理 Region 时，需要：
     - 将存活对象复制到新 Region
     - 修改对它的所有引用（Fix-up）
...
55. Java集合概览
    ...
    │   │   ├── TreeSet（红黑树、有序）
    │   │   └── CopyOnWriteArraySet（并发）/ ConcurrentHashMap.newKeySet
    │   │
    │   └── Queue（队列）
    │       ├── Deque（双端队列）
    ...
    └── Map（键值映射）
        ...
        └── WeakHashMap（弱引用 key）
    ```
56. CopyOnWriteArrayList 和 CopyOnWriteArraySet实现原理
    - CopyOnWriteArrayList 写操作：
        - 获取独占锁 
        - 复制当前数组 
        - 在新数组上修改 
        - 将新数组赋值给 array 
        - 释放锁
    - CopyOnWriteArraySet 本质上是 基于 CopyOnWriteArrayList 封装的 Set 容器，add时先在当前数组中查找是否存在
57. JDK11、17、21版本特性
    - JDK11：新的 HTTP/2 Client（标准 API）、ZGC（实验性）首次加入
    - JDK17：record、ZGC 从实验 → 稳定
    - JDK21：虚拟线程（Virtual Threads）、ZGC 大幅增强
58. 每个 Java 类都有一个引用指向加载它的 ClassLoader。不过,数组类不是通过 ClassLoader 创建的,
    而是 JVM 在需要的时候自动创建的,数组类通过getclassLoader()方法获取 ClassLoader 的时候和该数组的元素类型的 classLoader 是一致的。
59. 如果我们不想打破双亲委派模型,就重写ClassLoader 类中的findClass()方法即可,无法被父类加载器加载的类最终会通过这个方法被加载。
    但是,如果想打破双亲委派模型则需要重写 loadClass()方法。
60. Tomcat 打破双亲委派模型的原理:自定义了类加载器 WebAppClassLoader,每个 Web 应用都会创建一个单独的 WebAppClassLoader
61. 如何快速实现LRU
